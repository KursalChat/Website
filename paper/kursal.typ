...
It is computed by applying a key derivation function to the identity keys and signing keys of both parties in a deterministic way in order to get the same result on both sides. Both public identity keys are sorted and concatenated with the two sorted Dilithium-5 public signing keys. Those four concatenated keys are then hashed with the SHA-256 algorithm. The output is then formatted as 8 groups of 4 digits from the SHA-256 and displayed on the user interface.
...
== Malicious Relays
Relays, even malicious, will never be able to read your messages as they are encrypted. However, they can analyze your messaging patterns if unprotected. This is why peer IDs rotate regularly to prevent tracking your communication patterns. Additionally, relays have access to your IP address if you are not using a VPN.
...
This means that in order to intercept 0.1% of the DHT entries, you would have to calculate at $1000$ hashes per second for $1000$ years with $7.1 * 10^15$ devices. And for one interception in a million, it drops to $7.1 * 10^12$ devices for the next $1000$ years at $1000$ hashes per second. For more details about what DHT interceptions lead to, refer to @dht_interception.

The Argon2id configuration used is set to last a significant time on modern computers. We are therefore well under the theoretical 1000 hashes per second, making brute-forcing impractical and future-proof. Here are the results of the hashing benchmark (Kursal uses those exact settings) with:
...
#figure(
  table(
    ...
    // [Intel Core \ i5 14600kf], [1343 ms \ 67.55 ms (threaded)], "14.80", // ender

    "iMac M5", [376.2 ms \ 37.7 ms (threaded)], "26.53",
    // kubik
